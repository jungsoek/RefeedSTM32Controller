...
//	  LOG("[DEBUG]", (char*)LOG_buffer);
	...
	//			return CLOSES;
	//		} else if(strstr((char *)LOG_buffer, "POSTUCOA")){
	//			return POSTUCOA;
	...
	//		          loadcell_2_handshake = HX711_Get_Value(&scale2, 10, 2000);
				  ...
				  case VALIDATION:
					  ...
					  if (!SIMCom_Post(data_PostSimCom, url_identity, 5000)) {
						  // LOG("[DEBUG]", (char*)SIM_data);  // 필요하면 주석 풀고 확인 // If you need it, Uncomment and check the log.
				  ...
				  case OPEN_INPUT:
					  ...
					  HAL_Delay(1500);  // HX711 안정화 (200~500ms 권장)

					  // ----- Restore the original code after dummy data testing is completed -----
...
//					  ultra = Ultra_ReadDistance_Dummy(); // end of Ultra_ReadDistance() dummy data declaration.
					  ...
					  snprintf(data_TransmitHeader, sizeof(data_TransmitHeader),
							   "Ultra=%.2f cm", ultra);   // 단위가 cm 맞으면 cm, 아니면 mm로 바꿔
					  LOG("[ANS]", data_TransmitHeader);
...
//					  water_weight = WATER_ReadLastOn_Quick_Dummy(220);
				  ...
				  case SETTING:
					  ...
					  snprintf(data_TransmitHeader, sizeof(data_TransmitHeader),
							   "Container=%.2f L", value_VolContainer);   // 단위가 cm 맞으면 cm, 아니면 mm로 바꿔
					  LOG("[ANS]", data_TransmitHeader);
					  FUNCTION = SPACE;
					  break;
				  case SPACE:
